...
def run_simulation(ticker, start_date, freq, amt, output):
    ...
    calculate_contributions(start_date, freq, amt, tickerDf)
    ...
    for row in range(len(df.index)):
        if str(df.iloc[row]['Previous Return'].item()) == "nan":
            tot_bal.append(df.iloc[row]['Contribution'].item())
        else:
            bal = tot_bal[row - 1] * (1 + df.iloc[row]['Previous Return'].item()) + df.iloc[row]['Contribution'].item()
            tot_bal.append(bal)
